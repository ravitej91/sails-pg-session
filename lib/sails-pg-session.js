...
//  PostSQLStore.prototype.__proto__ = Store.prototype;
  ...
  PostSQLStore.prototype.get = function (sid, fn) {
	sails.log.debug('sails-pg-session#get - requesting a new connection from the pool...');
	pg.connect(__options__, function(err, client, done) {
	  try {
        sails.log.debug('sails-pg-session#get - ' + client.processID + '@' + client.poolCount + ': lookup session ' + sid + '...');
        client.query("SELECT data_out FROM sails_session_store_get($1)", [sid], function (err, result) {
            if (err) {
              ...
              sails.log.debug('sails-pg-session#get - ' + client.processID + '@' + client.poolCount + ': releasing connection...');
              done();
  ...
  PostSQLStore.prototype.set = function (sid, sess, fn) {
    sails.log.debug('sails-pg-session#set - requesting a new connection from the pool...');
	pg.connect(__options__, function(err, client, done) {
      try {
          sails.log.debug('sails-pg-session#set - ' + client.processID + '@' + client.poolCount + ': persisting session ' + sid + '...');
          client.query("SELECT sails_session_store_set($1,$2)", [sid, sess], function (err, result) {
            if (err) {
              ...
              sails.log.debug('sails-pg-session#set - ' + client.processID + '@' + client.poolCount + ': releasing connection...');
              done();
  ...
  PostSQLStore.prototype.destroy = function (sid, fn) {
    sails.log.debug('sails-pg-session#destroy - requesting a new connection from the pool...');
    pg.connect(__options__, function(err, client, done) {
      try {
        sails.log.debug('sails-pg-session#destroy - ' + client.processID + '@' + client.poolCount + ': desctroying session ' + sid + '...');
        client.query("SELECT sails_session_store_destroy($1)", [sid], function (err, result) {
          if (err) {
            ...
            sails.log.debug('sails-pg-session#destroy - ' + client.processID + '@' + client.poolCount + ': releasing connection...');
            done();
  ...
  PostSQLStore.prototype.length = function (fn) {
    pg.connect(__options__, function(err, client, done) {
      sails.log.debug('sails-pg-session#length - ' + client.processID + '@' + client.poolCount + ': requesting a new connection from the pool...');
      try {
        sails.log.debug('sails-pg-session#length - ' + client.processID + '@' + client.poolCount + ': lookup the number of sessions...');
        client.query("SELECT length FROM sails_session_store_length()", function (err, result) {
          if (err) {
            ...
            sails.log.debug('sails-pg-session#length - ' + client.processID + '@' + client.poolCount + ': releasign connection...');
            done();
  ...
  PostSQLStore.prototype.clear = function (fn) {
    pg.connect(__options__, function(err, client, done) {
      sails.log.debug('sails-pg-session#clear - ' + client.processID + '@' + client.poolCount + ': requesting a new connection from the pool...');
      try{
        sails.log.debug('sails-pg-session#clear - ' + client.processID + '@' + client.poolCount + ': clearing sessions...');
        client.query("SELECT sails_session_store_clear()", function (err, result) {
    	  if (err) {
            ...
            sails.log.debug('sails-pg-session#clear - ' + client.processID + '@' + client.poolCount + ': releasing connection...');
            done();
